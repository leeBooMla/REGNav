...
from src.model.renet import CNNEncoder, RelationNetwork, CNNEncoder_mp3d,RelationNetwork_mp3d,ConcatNetwork,copy_state_dict
from src.diffusion import SDFeaturizer
import clip
...
class EarlyFuseCNNEncoder_diff(nn.Module):
    def __init__(
        self,
        observation_space,
        baseplanes=32,
        ...
        if self.obs_transform is not None and not backbone == "clip_feat" and not backbone == "diffusion":
        ...
        self._rgb_aug = None
        if "rgb" in visual_uuid or "imagegoal_sensor_v2" in visual_uuid and not backbone == "clip_feat" and not backbone == "diffusion":
        ...
        if backbone == "clip":
            ...
            self.output_shape = 1024
        elif backbone == "diffusion":
            self.sd_encoder = SDFeaturizer()
            self.output_shape = 2*1280*8*8
        else:    
            if not self.is_blind:
                ...
                if pair_relation :
                    ...
                    # copy_state_dict('/data/lpn/room-expert/checkpoints/gibson_train_ckps/k2_6_size_128/feature_encoder.pkl', self.feature_encoder, strip='module.')
                    copy_state_dict('/data/lpn/room-expert/checkpoints/mp3d/feature_encoder57.pkl', self.feature_encoder, strip='module.')
                    self.relation_network = RelationNetwork()
                    copy_state_dict('/data/lpn/room-expert/checkpoints/mp3d/relation_network57.pkl', self.relation_network, strip='module.')
                    # copy_state_dict('/data/lpn/room-expert/checkpoints/gibson_train_ckps/k2_6_size_128/relation_network.pkl', self.relation_network, strip='module.')
                    # self.concat_network = ConcatNetwork()
    ...
    def forward(self, observations):
        if self.is_blind:
            ...
            x = observations['rgb'] 
        elif getattr(self, "_backbone") == "diffusion":    
            x_o = observations['rgb'].permute(0, 3, 1, 2) / (255.0- 0.5) * 2
            x_g = observations['imagegoal_sensor_v2'].permute(0, 3, 1, 2) / (255.0- 0.5) * 2
            f_o = self.sd_encoder.forward(x_o)
            f_g = self.sd_encoder.forward(x_g)
            x = torch.cat((f_o,f_g),dim=1)
            cam_visual = None
            return x, cam_visual
        else:    
            seeds = None
            ...
            if self._n_input["panoramic_rgb_sensor"] > 0:
                ...
                for i in range(n_panoramic):
                    ...
                    if self.pair_relation:
                        with torch.no_grad():
                            feature1 = self.feature_encoder(v_obs_imagegoal)
                            feature2 = self.feature_encoder(v_obs_panoramic[:,:,:,:,i])
                ...
                relation = torch.softmax(sum(relation),dim=1) 
                x = torch.cat(x, dim=1)

                return x, cam_visual, relation
            ...
            if self.cam_visual:
                l = self.encoder[1].stem[3]
                # l = self.encoder[1].stem_o.layers[3]
                cam_visual_o = visualize_cam(cnn_input, l, observations['rgb'])
                # l = self.encoder[1].room_encoder.conv
                # cam_visual_o_r = visualize_cam(cnn_input, l, observations['rgb'])
                l = self.encoder[1].stem[3]
                # l = self.encoder[1].stem_g.layers[3]
                cam_visual_g = visualize_cam(cnn_input, l, observations['imagegoal_sensor_v2'])
                # l = self.encoder[1].room_encoder.conv
                # cam_visual_g_r = visualize_cam(cnn_input, l, observations['imagegoal_sensor_v2'])
                cam_visual = np.concatenate((cam_visual_g, cam_visual_o), axis=1)
            ...
            if self.pair_relation:
                ...
                with torch.no_grad():
                    ...
                    relations = self.relation_network(relation_pairs)
                    relations = torch.softmax(relations, dim=1)
                # return x, cam_visual, relations
                # x = self.concat_network(x, relation_pairs)
...
class EarlyFuseCNNEncoder(nn.Module):
        ...
        if backbone == "clip":
            ...
            if not self.is_blind:
                ...
                if pair_relation :
                    self.feature_encoder = CNNEncoder()
                    copy_state_dict('/data/lpn/room-expert/checkpoints/gibson_train_ckps/k2_6_size_128/feature_encoder.pkl', self.feature_encoder, strip='module.')
                    # self.feature_encoder = CNNEncoder_mp3d()
                    # copy_state_dict('/data/lpn/room-expert/checkpoints/mp3d/feature_encoder57.pkl', self.feature_encoder, strip='module.')
                    self.relation_network = RelationNetwork()
                    copy_state_dict('/data/lpn/room-expert/checkpoints/gibson_train_ckps/k2_6_size_128/relation_network.pkl', self.relation_network, strip='module.')
                    # self.relation_network = RelationNetwork_mp3d()
                    # copy_state_dict('/data/lpn/room-expert/checkpoints/mp3d/relation_network57.pkl', self.relation_network, strip='module.')
                    # self.concat_network = ConcatNetwork()
    ...
    def forward(self, observations):
        if self.is_blind:
            ...
            random = 0
            mp3d = 0
            # feature_encoder_mp3d = CNNEncoder_mp3d()
            # copy_state_dict('/data/lpn/room-expert/checkpoints/mp3d/feature_encoder57.pkl', feature_encoder_mp3d, strip='module.')
            # relation_network_mp3d = RelationNetwork_mp3d()
            # copy_state_dict('/data/lpn/room-expert/checkpoints/mp3d/relation_network57.pkl', relation_network_mp3d, strip='module.')
            if self.pair_relation:
                if random == True:
                    b = x.shape[0]
                    # index = torch.randint(0,2,size=(b,1))
                    # relations = torch.zeros(b,2).scatter_(1,index,1)
                    relations = torch.rand(b,2)
                    relations = torch.softmax(relations.cuda(), dim=1)
                    return x, cam_visual, relations
                x_o = cnn_input[:,:3,...]
                x_g = cnn_input[:,3:,...]
                if mp3d:
                    return x, cam_visual, cnn_input
                    with torch.no_grad():
                        feature_encoder_mp3d = feature_encoder_mp3d.cuda()
                        relation_network_mp3d = relation_network_mp3d.cuda()
                        feature1 = feature_encoder_mp3d(x_o)
                        feature2 = feature_encoder_mp3d(x_g)
                        relation_pairs = torch.cat((feature1,feature2),dim=1)
                        relations = relation_network_mp3d(relation_pairs)
                        relations = torch.softmax(relations, dim=1)
