...
def load_model(model, dir):
    ...
    pretrained_dict = torch.load(dir,map_location=torch.device('cuda', torch.distributed.get_rank()))['params']
...
def parse_args(arg_mode):
    ...
    parser.add_argument('-dataset', type=str, default='mp3d',
                        choices=['miniimagenet', 'cub', 'tieredimagenet', 'cifar_fs', "mp3d"])
    parser.add_argument('-data_dir', type=str, default='datasets', help='dir of datasets')

    ''' about training specs '''
    parser.add_argument('-batch', type=int, default=8, help='auxiliary batch size')
    parser.add_argument('-temperature', type=float, default=0.2, metavar='tau', help='temperature for metric-based loss')
    parser.add_argument('-lamb', type=float, default=0.5, metavar='lambda', help='loss balancing term')
    ...
    parser.add_argument('-milestones', nargs='+', type=int, default=[40, 50], help='milestones for MultiStepLR')
    ...
    parser.add_argument('-shot', type=int, default=30, metavar='K', help='number of shots')
    parser.add_argument('-query', type=int, default=10, help='number of query image per class')
